...
void *msg_handler(void *socket_desc)
{
 ...
 //   write(socket, msg, strlen(msg));
    ...
    while ((size = recv(socket, cli_message, 2000, 0)) > 0)
    {
		...
		//syslog(LOG_INFO,"Row data %d with data: %x\n",row,row_data[row]);
            }
	    syslog(LOG_INFO,"Row data 1 with data: %x\n",row_data[0]);
	    syslog(LOG_INFO,"Row data 2 with data: %x\n",row_data[1]);
	    syslog(LOG_INFO,"Row data 3 with data: %x\n",row_data[2]);
	    syslog(LOG_INFO,"Row data 4 with data: %x\n",row_data[3]);
