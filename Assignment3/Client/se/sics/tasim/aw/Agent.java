 ...
 * Updated : $Date: 2004/11/11 21:46:28 $
 *           $Revision: 1.11 $
 */
...
* implementations of agents that wants to be able to participate in
* TAC Games or other Trading Agent Simulations on the TAC SCM Simulator.
*
* Agents can be run both within the Simulator as built-in agents or they
* can run outside the Simulator (in the AgentWare) and connect to it via
* Internet.
*
* Features of Agent:
* <ul>
...
* </ul>
*
* Note: the agent must wait for its initialization before calling any
* of the inherited methods!  The agent is initialized by a call to
...
public abstract class Agent {
   ...
   * <p>
   *
   * This method is package protected to ensure that only AgentService
   * can call it.
   *
   * @param service the <code>AgentService</code> to use for this agent
   */
  ...
  /**
   * Returns the server time. Note that this is an approximation when
   * agent is not executing as "built-in".
   */
  ...
  /**
   * Adds the specified time listener to receive notifications about
   * time units.
   *
   * @param listener the time listener
   */
  ...
  /**
   * Extract all receivers and message contents from the hash table and
   * sends them.
   *
   * @param messageTable a <code>Hashtable</code> mapping agent
   *	addresses (receivers) with Transportable objects (message contents)
   */
   ...
   * The agent might still receive messages after this call has been
   * made but can not send messages itself.
   */
