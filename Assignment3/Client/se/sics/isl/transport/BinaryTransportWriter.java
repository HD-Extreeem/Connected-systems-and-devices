 ...
 * Updated : $Date: 2005/06/08 20:25:50 $
 *           $Revision: 1.7 $
 * Purpose :
...
//     System.out.println("FLOAT Attribute: " + name);
  ...
  private void writeCurrentNode(byte nodeType, int pos) {
	...
	if (isTablesSupported
	    && nodeName[other] == name && nodeWritten[other]
...
// 		       + Integer.toHexString(data) + ") to " + nrOfBytes);
  ...
  private void writeString(String value) {
    ...
    if (nrOfBytes + maxSize > byteData.length) {
      byteData = (byte[])
	ArrayUtils.setSize(byteData, nrOfBytes + maxSize + DATA_SIZE);
    }
  ...
  private void writeAlias(int id, String name) {
    ...
    if (aliasSize + maxSize > aliasData.length) {
      aliasData = (byte[])
	ArrayUtils.setSize(aliasData, aliasSize + maxSize + ALIAS_SIZE);
    }
